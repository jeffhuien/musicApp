...
<template>
  <view class="w-full relative flex max-md:ml-0 text-xs">
    ...
    <input
      type="text"
      class="dark:bg-gray-800 transition-all duration-700 w-52 focus:w-64 rounded-3xl border border-solid border-pink-500 pl-8 py-2 max-sm:py-1 text-xs text-gray-600 focus:outline-sky-200 hover:border-lime-300 outline-none"
      v-model="keyword"
      :placeholder="Text"
    ...
    <view v-show="show" class="overflow-hidden z-[51] flex max-sm:scale-90 suggest min-h-[15rem] absolute top-[120%] max-sm:top-2 max-sm:-left-2 w-72 bg-white dark:bg-gray-800 rounded-md shadow-lg">
      <view v-if="data?.result" class="h-full w-full absolute">
        <view v-if="data.result.albums" class="flex flex-col gap-2 p-2 h-full border border-solid dark:border-gray-700 dark:bg-gray-800 bg-white">
